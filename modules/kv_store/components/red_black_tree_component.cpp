#include "red_black_tree_component.h"
#include "modules/kv_store/event_names.h"
...
RedBlackTreeComponent::RedBlackTreeComponent() {
	TNULL = new RBTNode;
	TNULL->color = 0;
	TNULL->left = nullptr;
	TNULL->right = nullptr;
	...
	//TODO: get database name from config remove hard-coding
...
bool verify_kv_dict(Variant p_data) {
	...
	p_data.has_key("key" , has_key);
	p_data.has_key("value", has_value);
	p_data.has_key("wal"  , has_wal);
...
NodePtr RedBlackTreeComponent::get_tnull() {
	return this->TNULL;
}

NodePtr RedBlackTreeComponent::search_tree(uint64_t p_key) {
	return _search_tree(this->root, p_key);
}

void RedBlackTreeComponent::insert_node(uint64_t p_key, Variant p_value, bool p_wal) {
...
void RedBlackTreeComponent::delete_node(uint64_t p_key, bool p_wal) {
	_delete_node(this->root, p_key, p_wal);
}

NodePtr RedBlackTreeComponent::minimum(NodePtr p_node) {
...
NodePtr RedBlackTreeComponent::maximum(NodePtr p_node) {
...
void RedBlackTreeComponent::_store_wal_entry(NodePtr p_node, ACTION_TYPE p_action) const {
...
void RedBlackTreeComponent::_run_wal_file() {
...
void RedBlackTreeComponent::_reset_wal_file() const {
...
void RedBlackTreeComponent::_init_null_node(NodePtr p_node, NodePtr p_parent) {
	p_node->key = 0;
...
NodePtr RedBlackTreeComponent::_search_tree(NodePtr p_node, uint64_t p_key) const {
...
void RedBlackTreeComponent::_fix_insert(NodePtr p_node) {
	NodePtr u;
...
void RedBlackTreeComponent::_left_rotate(NodePtr p_node) {
...
void RedBlackTreeComponent::_right_rotate(NodePtr p_node) {
...
void RedBlackTreeComponent::_fix_delete(NodePtr p_node) {
	NodePtr s;
...
void RedBlackTreeComponent::_rb_transplant(NodePtr u, NodePtr v) {
	if (u->parent == nullptr) {
		root = v;
...
void RedBlackTreeComponent::_delete_node(NodePtr p_node, uint64_t p_key, bool p_wal) {
	NodePtr z = TNULL;
	NodePtr x, y;
	while (p_node != TNULL) {
		if (p_node->key == p_key) {
